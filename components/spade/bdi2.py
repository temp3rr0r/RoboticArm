...
class PeriodicSenderAgent2(Agent):
...
class ReceiverAgent(Agent):
    class RecvBehav(CyclicBehaviour):
        async def run(self):
            print("-- Receiver Agent: RecvBehav running")
            msg = await self.receive(timeout=10)  # wait for a message for 10 seconds
            # msg = await self.receive(timeout=3)  # wait for a message for 10 seconds
            if msg:
                print("-- Receiver Agent: Message received with content: {}".format(msg.body))
            else:
                print("-- Receiver Agent: Did not received any message after 10 seconds")
                self.kill()
    ...
    async def setup(self):
        print("-- Receiver Agent: ReceiverAgent started")
        b = self.RecvBehav()
        self.add_behaviour(b)
