...
def pyhop(state, tasks, verbose=0, all_plans=False):
    ...
    results = seek_plans(state, tasks, [], 0, verbose, all_plans)
...
def seek_plans(state, tasks, plan, depth, verbose=0, all_plans=False):
    """
    ...
    if task1[0] in operators:
        ...
        if newstate:
            solution = seek_plans(newstate, tasks[1:], plan + [task1], depth + 1, verbose)
            if solution != False:
               if all_plans:
    ...
    if task1[0] in methods:
        ...
        for method in relevant:
            ...
            if subtasks != False:
                solution = seek_plans(state, subtasks + tasks[1:], plan, depth + 1, verbose)
                if solution != False:
                    if all_plans:
                        plans.append(solution)
                    else:
...
# TODO: return all possible plans
